  ...
  return subscribe({
    ...
    subscribe: {
      ...
      toggleProductSpotlightBegin: function(msg) {
        ...
        var existing = _.findWhere(this.result.products, { id: msg.id });
        existing.inSpotlight = !existing.inSpotlight;
        setTimeout(function() {
          this.publish('toggleProductSpotlightEnd', { product: existing });
        }.bind(this), 1000);
      },
      toggleProductGalleryBegin: function(msg) {
        var products = this.result.products;
        var existing = _.findWhere(this.result.products, { id: msg.id });
        existing.inGallery = !existing.inGallery;
        setTimeout(function() {
          this.publish('toggleProductGalleryEnd', { product: existing });
        }.bind(this), 1000);
      }
    }
  });
