define(function(require) {
  ...
  var ProductFilter = function(config) {
    ...
    //hide products not at the current location
    //(isAvailable == false)
    this.hideUnavailable = config.hideUnavailable;
  };
  ...
  _.extend(ProductFilter.prototype, {
    ...
    filterProducts: function(products) {
      ...
      var hideUnavailable = this.hideUnavailable;
    ...
    getCategorySelectView: function(sections) {
      ...
      //if no filter, match first selection
      if(!matched) {
        if(this._categoryFilter == UNCATEGORIZED) {
          ...
          this._categoryFilter = result[0].value;
        }
      }
      return result;
    }
