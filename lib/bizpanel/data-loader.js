define(function(require) {
  ...
  var ctrl = {
    subscribe: {
      changePasswordBegin: function(msg) {
        var self = this;
        var payload ={
          password: msg.password
        };
        troller.spinner.spin();
        api.users.update(self.data.user.id, payload, function(err, res) {
          troller.spinner.stop();
          if(err) self.publish('showError', {message: 'There was a problem changing your password.' });
          self.publish('changePasswordEnd');
        });
      },
