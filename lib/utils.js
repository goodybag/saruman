define(function(require){
  ...
  //object holding "differences" between goodybag and bizpanel behavior
  utils.context = {
    isBizPanel: function() {
      return !!~window.location.toString().indexOf('bizpanel');
    },
    //called by the app after login
    //to get an intial redirection
    getHome: function(user) {
      if(user.isManager()) {
        return {
          name: 'dashboard',
          options: {},
  ...
  var placeholderImage = 'https://www.filepicker.io/api/file/Akx3DdFIQySrNhfkfzZI';
  var convertSegment = '/convert?'

  //extracts the url to a photo from a full url
  //which can contain resizing information
  //basically returns the value passed into utils.getSizedPhotoUrl
  utils.getRawPhotoUrl = function(sizedPhotoUrl) {
    //don't return a url if its the placeholder image
    if(sizedPhotoUrl.indexOf(placeholderImage) == 0) {
      return null;
    }
    return sizedPhotoUrl.split(convertSegment)[0];
  };

  //returns a sized photo url
  //if the url is null it will be replaced with a default 'place holder' image
  utils.getSizedPhotoUrl = function(photoUrl, size) {
    var url = photoUrl || placeholderImage;
    return url + convertSegment + 'w=' + size + '&h=' + size;
  };
